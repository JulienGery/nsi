...
for row in rows:
    ...
    tour+=1
    for nbcarre in range(row):    #création de la ligne et possitionnement de l'axe x. nbcarre est le numéro du carré en cour sur la rangée
        t.goto(nbcarre*(size+size/3), posY) #nbcarre*(size+size/3) est le début du carré.
        t.pendown()
